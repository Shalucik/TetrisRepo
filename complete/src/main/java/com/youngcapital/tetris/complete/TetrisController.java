...
public class TetrisController {
	...
	public Greeting initGreeting() {
		...
		return new Greeting(new Pos[] {}, currentPositions, currentBlock.getColor());
	}

	public Pos[] createBlock() {
		currentBlock = getRandomBlock();
		return getBlockPoints(currentBlock);
	}

	private Pos[] getBlockPoints(Block block) {
		...
		points[0] = new Pos(ors[orientation].getPosition0().getX() + offX,
				ors[orientation].getPosition0().getY() + offY);

		points[1] = new Pos(ors[orientation].getPosition1().getX() + offX,
				ors[orientation].getPosition1().getY() + offY);

		points[2] = new Pos(ors[orientation].getPosition2().getX() + offX,
				ors[orientation].getPosition2().getY() + offY);

		points[3] = new Pos(ors[orientation].getPosition3().getX() + offX,
	...
	public Greeting moveGreeting(MoveMessage message) {
		...
		Pos move = new Pos(message.getX(), message.getY());
		Pos[] newPositions = new Pos[currentPositions.length];
		for (int i = 0; i < newPositions.length; i++) {
			newPositions[i] = new Pos(currentPositions[i].getX() + message.getX(),
					currentPositions[i].getY() + message.getY());
			if ((message.getX() != 0 && checkGrid(newPositions[i]))) {
				return new Greeting();
			}
			if (newPositions[i].getY() == grid.length || checkGrid(newPositions[i])) {
				updateGrid();
				currentPositions = createBlock();
				return new Greeting(checkForLines(), currentPositions, "gray");
			}
		}
	...
	private Pos[] checkForLines() {
		ArrayList<Pos> linePositions = new ArrayList<>();
		...
		return linePositions.toArray(new Pos[0]);
		
	}

	public boolean checkGrid(Pos point) {
